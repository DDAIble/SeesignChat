...
- RAG 청크 헤더 \`#### [N] 파일명 / 시트명 / 행 X~Y\` 형식 — **각 불릿·소주제 끝**에 \`[근거 N]\` 를 **반드시** 표기하세요 (N = 청크 헤더 번호). 파일명·행 텍스트를 직접 쓰지 마세요.
- 여러 청크 근거면 \`[근거 1] [근거 3]\` 처럼 번호만 나열하세요. 화면에서 클릭하면 본문이 표시됩니다.
- **paraphrase(요약) 문장**은 따옴표·인용 블록 없이 서술하되, 문장 끝에 \`(AI 요약)\` 과 \`[근거 N]\` 를 **함께** 붙이세요.
...
- **exact** (정확 일치): 파일/시트/행 설명 후 \`[근거 N]\` 표기 (N = 출처 추적 검색 섹션 #### [N] 번호). 본문 발췌는 답변에 길게 붙이지 마세요 — \`[근거 N]\` 클릭으로 확인합니다.
- **similar** (유사): "AI 요약과 가장 가까운 원문"임을 명시하고 \`(AI 요약)\` + \`[근거 N]\` 으로 유사 게시글 위치를 안내하세요.
...
- 답변 본문에는 **[1], [2]** 같은 단순 인용 번호를 쓰지 마세요. 대신 근거가 필요한 불릿 끝에 **`[근거 N]`** (N = RAG·출처 검색 섹션 번호)을 붙이세요 — 화면에서 클릭하면 본문 모달이 열립니다.
