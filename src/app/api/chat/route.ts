...
- **paraphrase(요약) 문장**은 따옴표 없이 서술하고 문장 끝에 **\`[근거 N:52,58]\` 태그 하나**만 붙이세요. \`(AI 요약)\`·\`[출처 N건]\`·\`#evidence-\` 문구는 쓰지 마세요.
...
- **similar** (유사): \`[근거 N:행번호,...]\` **태그 하나**만 표기.
...
## 답변 형식 — 최우선 (GFM 마크다운)
- 답변은 **유효한 GFM 마크다운**만 출력하세요. 앱이 마크다운을 HTML로 렌더링합니다.
- **## / ### 제목**, **GFM 표**(\`| 열 |\`), **--- 구분선**, **\`\`\`chart JSON** 은 각각 **독립된 줄**에 쓰고, **앞에 빈 줄 1줄**을 넣으세요.
- 문장 끝 \`[근거 N]\` 태그 **다음 줄**에서 제목·표·구분선을 시작하세요. **절대 금지**: \`[근거 N]###\`, \`출처 200건|\`, \`입니다.###\` 처럼 같은 줄에 붙이기.
- 차트는 반드시 \`\`\`chart ... \`\`\` 펜스로만 출력하세요. 텍스트로 60% 23% 나열 금지.
- 수치 근거는 문장 끝에 \`[근거 N]\` 태그만 붙이세요 (화면이 **[출처 N건]** 버튼으로 변환).
