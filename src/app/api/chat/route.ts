...
- 화면에서는 **「근거 원문 (N건)」** 클릭 링크 **하나**로 변환되며, 클릭 시 참조한 **모든 행**이 모달에 함께 표시됩니다.
...
- **exact** (정확 일치): \`[근거 N:행번호,...]\` **태그 하나**만 표기. 본문·파일명 텍스트는 답변에 쓰지 마세요.
...
- 답변 본문에는 **[1], [2]** 같은 단순 인용 번호를 쓰지 마세요. 근거는 **한 문장당** \`[근거 N:52,58]\` 또는 \`[근거 2:52;3:71]\` **태그 하나**만 사용하세요.
