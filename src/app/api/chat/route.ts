...
- RAG 청크 헤더 \`#### [N] 파일명 / 시트명 / 행 X~Y\` — **각 불릿 끝**에 \`[근거 N]\` 표기 (N = 청크 번호). 화면에서는 **파일명·행 번호** 링크로 자동 변환되며 클릭 시 본문 모달이 열립니다.
- 여러 청크 근거면 \`[근거 1] [근거 3]\` 처럼 번호만 나열하세요.
...
- 답변 본문에는 **[1], [2]** 같은 단순 인용 번호를 쓰지 마세요. 대신 \`[근거 N]\` (N = RAG·출처 검색 섹션 #### [N] 번호)을 붙이세요 — 화면에서 **파일명·행 번호** 클릭 링크로 변환됩니다.
